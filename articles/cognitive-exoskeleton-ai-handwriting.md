...
しかし、著名OSS（たとえば NousResearch/Hermes-Agent へのWatchdog実装やプロセス管理PRなど）への継続的なコントリビューション、AIコードレビュー、回帰テスト、CI、セキュリティ境界の検討まで含めて開発を行っている人に対し、主な評価軸として「AIを外して手打ちできるか」を置くのは、現在の開発実務を正確に測っているのでしょうか。
...
* **MCP（Model Context Protocol）の Sequential Thinking** で問題分解を補助する
* **Context7 等** で一次資料や現行ドキュメントを参照する
...
今回、私がコーディングエージェントへ毎回読ませているSOP群を数えると、**20ファイル、約800行**ありました。
...
| **Sequential Thinking** | 問題分解と段階的な検討 | ワーキングメモリ上の同時保持を減らす |
| **Context7 等** | 現行ドキュメントの参照 | 記憶や思い込みだけに依存しない |
...
航空機のパイロットに対して、「計器、チェックリスト、航法装置、自動操縦を全部切ってから能力を証明せよ」とは言いません。重要なのは、計器を正しく理解し、異常を早期に検出し、必要なら自動系をオーバーライドして、安全な判断を下せることです。
...
「AIを使ったか、使わなかったか」という幼稚な二分法ではなく、次を問うべきです。
...
企業側のセキュリティやガバナンスへの懸念についても、Enterprise契約の締結、ローカルモデルの併用、機密データの投入制限、アクセス制御、監査ログの保持など、技術的・運用的に解決する手段はすでに揃っています。
...
これからのAI時代における採用や就労支援が真に向き合うべきなのは、間違いなく後者であると信じています。
