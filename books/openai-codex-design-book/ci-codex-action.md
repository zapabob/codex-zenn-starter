...
## なぜ codex exec を直叩きしないか

CI から `npx @openai/codex` を自分で入れ、`OPENAI_API_KEY` を export する方法もあります。

しかし GitHub Actions では **`openai/codex-action@v1`** が推奨されます。
...
2. `secrets.OPENAI_API_KEY` をリポジトリ secrets に登録
...
- [ ] `openai/codex-action@v1` の最小ワークフローを書ける
