...
| 最新リリース（調査時点） | `rust-v0.140.0` 系 | `v3.2.0`（2026-05-22） |
| 同期基準 | — | 公式 `rust-v0.133.0` + upstream main |

zapabob/codex の `CHANGELOG.md` によると、v3.2.0 では次が行われています。

- 公式 Codex `rust-v0.133.0` および upstream main との同期
- Git4D app-server ブリッジ（AR/VR セッション API）
- `scripts/upstream_overlay_merge.py` による overlay merge ドライバ
- 公式セキュリティアドバイザリ `GHSA-w5fx-fh39-j5rw` のレビュー
...
この時点の本（5章構成）は、次の土台までをカバーします。

1. なぜ Codex を「開発環境設計」の問題として捉えるか
2. CLI を安全に導入する具体手順
3. AGENTS.md によるリポジトリ規律
4. Sandbox / Approval による権限設計
5. フォークを題材にした拡張と upstream 追従

今後追加予定の章（MCP、GitHub Actions、Windows-first 検証ログ、AGENTS.md テンプレート集）は、この5章の上に載せます。

Codexを起動する本ではなく、**Codexが安全に働ける開発環境を作る本** —— 第0章で述べたゴールは、ここまでの章で一段具体化されています。
